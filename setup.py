...
if platform.system() == "Linux":
    ...
    if os.path.islink(pathSym) == False:
        subprocess.run([f"echo creating symlink for {repoParDir}/{name} {symParDir}/{name}"], shell=True)
        subprocess.run([f"ln -s {repoParDir}/{name} {symParDir}/{name}"], shell=True)
